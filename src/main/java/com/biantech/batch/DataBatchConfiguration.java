package com.biantech.batch;

import com.biantech.aop.AopLogHandler;
import com.biantech.listener.PabJobListener;
import com.biantech.listener.PabStepListener;
...
public class DataBatchConfiguration {
    ...
    @Resource
    private PabJobListener jobListener;            //简单的JOB listener
...
//                ...
    ...
    public Step handleDataStep() {
        return stepBuilderFactory.get("getData").
                <Access, Access>chunk(6).        // <输入,输出> 。chunk通俗的讲类似于SQL的commit; 这里表示处理(processor)100条后写入(writer)一次。
                ...
                .listener(new PabStepListener())         //指定ItemWriter
                .listener(new AopLogHandler())
                .build();
    }
    ...
    public ItemReader<? extends Access> getDataReader() {
        ...
        try {
            BatchJpaNativeQueryProvider<Access> queryProvider = new BatchJpaNativeQueryProvider<>();
    ...
    public ItemProcessor<Access, Access> getDataProcessor() {
        ItemProcessor processor= new ItemProcessor<Access, Access>() {
            @Override
            public Access process(Access access) throws Exception {
                if(access.getId()%2==0) {
                    ...
                    log.info("processor data return null " );
                    return null;
                }
            }
        };
...
//        };
    ...
    public ItemWriter<Access> getDataWriter() {
        ItemWriter itemWriter=new ItemWriter<Access>(){
            @Override
            public void write(List<? extends Access> list) throws Exception {
                log.info("write batch data! size="+list.size());
